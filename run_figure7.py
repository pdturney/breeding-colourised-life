...
# ==========
#
# Assign numbers to each colour, according to the Immigration rule.
# The Immigration Game has three states (colours):
#
white  = 0
red    = 1
blue   = 2
...
prob_red =        0.15  # probability of red
prob_blue =       0.15  # probability of blue
prob_mutation =   0.05  # probability of switching among white, red, blue
...
for i in range(population_size):
  ...
  for sample in range(sample_size):
    ...
    seed_matrix = colour.make_seed_matrix(prob_red, prob_blue)
    # - grow the matrix
    adult_matrix = colour.grow_matrix(seed_matrix, num_steps)
...
for j in range(new_matrices):
  ...
  if (score1 > score2):
    ...
    seed4 = colour.mutate_seed(seed3, prob_mutation)
    adult2 = colour.grow_matrix(seed4, num_steps)
    ...
    seed4 = colour.mutate_seed(seed3, prob_mutation)
    adult1 = colour.grow_matrix(seed4, num_steps)
...
data_file = open("top_result_figure7.bin", "wb")
